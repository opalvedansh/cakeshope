...
import { ChefHat, Clock, Palette, Layers, ShoppingCart, Star, Plus, Minus } from "lucide-react";
...
export default function CustomOrder() {
  ...
  const cakeTypes = [
    { id: "round", name: "Round Cake", price: 45, image: "https://images.unsplash.com/photo-1578985545062-69928b1d9587?w=300&h=200&fit=crop" },
    { id: "square", name: "Square Cake", price: 50, image: "https://images.unsplash.com/photo-1558618666-fcd25c85cd64?w=300&h=200&fit=crop" },
    { id: "tiered", name: "Tiered Cake", price: 120, image: "https://images.unsplash.com/photo-1464349095431-e9a21285b5f3?w=300&h=200&fit=crop" },
    { id: "cupcakes", name: "Cupcake Set (12)", price: 36, image: "https://images.unsplash.com/photo-1587668178277-295251f900ce?w=300&h=200&fit=crop" }
  ];
  ...
  const calculatePrice = () => {
    ...
    // Base cake price
    const selectedCakeType = cakeTypes.find(type => type.id === orderData.cakeType);
    ...
    // Flavor price
    const selectedFlavor = flavors.find(flavor => flavor.id === orderData.flavor);
    ...
    // Frosting price
    const selectedFrosting = frostings.find(frosting => frosting.id === orderData.frosting);
  ...
  const toggleArrayField = (field: 'filling' | 'decorations' | 'colors', value: string) => {
    setOrderData(prev => ({
      ...prev,
            ...
            <div className="max-w-3xl mx-auto">
              <div className="mb-6">
                ...
                <p className="text-xl text-muted-foreground">
                  Design a completely custom cake with your choice of flavors, decorations, and special touches. 
                  Our master bakers will bring your vision to life.
                </p>
              </div>
            ...
            <div className="lg:col-span-2 space-y-8">
                ...
                <CardContent>
                  <div className="grid sm:grid-cols-2 gap-4">
                    {cakeTypes.map((type) => (
                      ...
                      >
                        <img src={type.image} alt={type.name} className="w-full h-32 object-cover rounded-t-lg" />
                        <div className="p-4">
                          <h3 className="font-semibold">{type.name}</h3>
                          <p className="text-sm text-muted-foreground">Starting at ${type.price}</p>
                        </div>
                      </div>
                    ))}
                  </div>
              ...
              {orderData.cakeType && orderData.cakeType !== 'cupcakes' && (
                <Card>
                  ...
                  <CardContent>
                    <RadioGroup value={orderData.size} onValueChange={(value) => updateOrderData('size', value)}>
                      <div className="grid sm:grid-cols-2 gap-4">
                        {sizes.map((size) => (
                          <div key={size.id} className="flex items-center space-x-2 p-4 rounded-lg border">
                            <RadioGroupItem value={size.id} id={size.id} />
                            <Label htmlFor={size.id} className="flex-1 cursor-pointer">
                              <div className="flex justify-between items-center">
                                <div>
                                  <div className="font-medium">{size.name}</div>
                                  <div className="text-sm text-muted-foreground">Serves {size.servings}</div>
                                </div>
              ...
              {orderData.cakeType && (
                <Card>
                  ...
                  <CardContent>
                    <RadioGroup value={orderData.flavor} onValueChange={(value) => updateOrderData('flavor', value)}>
                      <div className="grid sm:grid-cols-2 gap-4">
                        {flavors.map((flavor) => (
                          <div key={flavor.id} className="flex items-center space-x-2 p-4 rounded-lg border">
              ...
              {orderData.flavor && (
                <Card>
                  <CardHeader>
                    ...
                    <p className="text-sm text-muted-foreground">Select multiple fillings for extra flavor layers</p>
                  </CardHeader>
                  <CardContent>
                    <div className="grid sm:grid-cols-2 gap-4">
                      {fillings.map((filling) => (
                        <div key={filling.id} className="flex items-center space-x-2 p-4 rounded-lg border">
                          <Checkbox
                            id={filling.id}
                            checked={orderData.filling.includes(filling.id)}
                            onCheckedChange={() => toggleArrayField('filling', filling.id)}
                          />
                          <Label htmlFor={filling.id} className="flex-1 cursor-pointer">
                            <div className="flex justify-between items-center">
              ...
              {orderData.flavor && (
                <Card>
                  ...
                  <CardContent>
                    <RadioGroup value={orderData.frosting} onValueChange={(value) => updateOrderData('frosting', value)}>
                      <div className="space-y-3">
                        {frostings.map((frosting) => (
                          <div key={frosting.id} className="flex items-center space-x-2 p-4 rounded-lg border">
                            <RadioGroupItem value={frosting.id} id={frosting.id} />
                            <Label htmlFor={frosting.id} className="flex-1 cursor-pointer">
                              <div className="flex justify-between items-center">
                  ...
                  <CardContent>
                    <div className="grid sm:grid-cols-2 gap-4">
                      {decorations.map((decoration) => (
                        <div key={decoration.id} className="flex items-center space-x-2 p-4 rounded-lg border">
                          <Checkbox
                            id={decoration.id}
                            checked={orderData.decorations.includes(decoration.id)}
                            onCheckedChange={() => toggleArrayField('decorations', decoration.id)}
                          />
              ...
              {orderData.frosting && (
                <Card>
                  ...
                  <CardContent className="space-y-4">
                    <div>
                      <Label htmlFor="message">Message on cake (e.g., "Happy Birthday Sarah!")</Label>
                      <Input
                        id="message"
                  ...
                  {orderData.cakeType && (
                    <div className="space-y-3">
                      <div className="flex justify-between">
                        <span>Base Cake</span>
                        <span>${cakeTypes.find(t => t.id === orderData.cakeType)?.price || 0}</span>
                      </div>
                      
                      {orderData.size && (
                        <div className="flex justify-between">
                          <span>Size ({sizes.find(s => s.id === orderData.size)?.name})</span>
                          <span>+${sizes.find(s => s.id === orderData.size)?.price || 0}</span>
                        </div>
                      )}
                      ...
                      {orderData.flavor && (
                        <div className="flex justify-between">
                          <span>Flavor</span>
                          <span>+${flavors.find(f => f.id === orderData.flavor)?.price || 0}</span>
                        </div>
                      )}
                      ...
                      {orderData.filling.length > 0 && (
                        <div className="flex justify-between">
                          ...
                          <span>+${orderData.filling.reduce((sum, fillingId) => {
                            const filling = fillings.find(f => f.id === fillingId);
                      ...
                      {orderData.frosting && (
                        <div className="flex justify-between">
                          <span>Frosting</span>
                          <span>+${frostings.find(f => f.id === orderData.frosting)?.price || 0}</span>
                        </div>
                      )}
                      ...
                      {orderData.decorations.length > 0 && (
                        <div className="flex justify-between">
                          ...
                          <span>+${orderData.decorations.reduce((sum, decorationId) => {
                            const decoration = decorations.find(d => d.id === decorationId);
                      ...
                      <div className="flex justify-between text-lg font-bold">
                        <span>Total</span>
                        <span className="text-cake-pink">${calculatePrice()}</span>
                      </div>
                      
                      <Button
                        className="w-full bg-cake-pink hover:bg-cake-pink/90"
                        onClick={handleAddToCart}
                        disabled={!orderData.cakeType || !orderData.flavor || !orderData.frosting}
                      >
                      ...
                      <div className="text-xs text-muted-foreground text-center">
                        Final price may vary based on complexity. We'll contact you to confirm details and pricing.
                      </div>
                    </div>
                  )}
